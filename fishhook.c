// fishhook.c — Facebook fishhook (MIT License)
// https://github.com/facebook/fishhook
// Copyright (c) 2013, Facebook, Inc.
// Adapté pour ARC/arm64 iOS.
...
static vm_prot_t get_protection(void *sectionStart) {
    ...
    kern_return_t info_ret = vm_region_64(task, &address, &size,
                                          VM_REGION_BASIC_INFO_64,
                                          (vm_region_info_64_t)&info, &count, &object);
#else
    mach_msg_type_number_t count = VM_REGION_BASIC_INFO_COUNT;
    vm_region_basic_info_data_t info;
    kern_return_t info_ret = vm_region(task, &address, &size,
                                       VM_REGION_BASIC_INFO,
                                       (vm_region_info_t)&info, &count, &object);
#endif
    if (info_ret == KERN_SUCCESS) return info.protection;
    return VM_PROT_READ;
}
...
static void perform_rebinding_with_section(struct rebindings_entry *rebindings,
    ...
    const bool data_const = strcmp(section->segname, SEG_DATA_CONST) == 0;
